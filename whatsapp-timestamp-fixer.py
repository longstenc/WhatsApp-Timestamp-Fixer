...
def resolver_conflito(
    ...
    console.print(
        f"\n[yellow]⚠️[/] O arquivo [cyan]'{nome_arquivo}'[/] ja existe no diretorio de saida."
    )
    escolha = Prompt.ask(
        "   [bold][S][/]obrescrever este  [bold][T][/]odos  [bold][P][/]ular este  [bold][Q][/]ular todos",
        choices=["S", "T", "P", "Q"],
        default="S",
        show_choices=False,
    )
...
def main() -> None:
    ...
    # ── Diretorio de origem ────────────────────────────────────────────────────
    while True:
        origem_str = Prompt.ask("[bold yellow]📁[/] Diretorio de origem")
        dir_origem = Path(origem_str).resolve()
        if dir_origem.is_dir():
            break
        console.print("[bold red]❌[/] Diretorio invalido. Tente novamente.")

    # ── Diretorio de saida ─────────────────────────────────────────────────────
    while True:
        saida_str = Prompt.ask("[bold yellow]📁[/] Diretorio de saida")
        dir_saida = Path(saida_str).resolve()

        if dir_origem == dir_saida:
            console.print(
                "[bold red]❌[/] O diretorio de saida deve ser diferente do de origem."
            )
            continue

        if not dir_saida.exists():
            criar = Confirm.ask(
                f"[yellow]⚠️[/] O diretorio '[cyan]{dir_saida}[/]' nao existe. Deseja cria-lo?",
                default=True,
            )
            if criar:
            ...
            console.print("[bold red]❌[/] Caminho invalido. Tente novamente.")

    # ── Buscar arquivos (nao recursivo) ────────────────────────────────────────
    console.print()
    with console.status("[bold green]🔍 Escaneando arquivos..."):
    ...
    if not arquivos_origem:
        console.print("[bold yellow]⚠️[/] Nenhum arquivo .jpg, .jpeg ou .mp4 encontrado.")
        return
    ...
    console.print()
    console.print(f"[bold]📂[/] Arquivos encontrados: [cyan]{len(arquivos_origem)}[/]")
    console.print(f"   🖼️ Imagens ([cyan].jpg[/]): [green]{qtd_img}[/]")
    console.print(f"   🎬 Videos ([cyan].mp4[/]): [green]{qtd_vid}[/]")
    ...
    if conflitos_existentes:
        console.print(
            f"[yellow]⚠️[/] [cyan]{len(conflitos_existentes)}[/] arquivo(s) ja existe(m) no destino."
        )
    else:
        console.print("[green]✅[/] Nenhum conflito - diretorio de saida vazio.")

    # ── Confirmacao final antes de iniciar ──────────────────────────────────────
    console.print()
    if not Confirm.ask("[bold]🚀 Processar arquivos?[/]", default=True):
        ...
        for arquivo_origem in arquivos_origem:
            ...
            if dados is None:
                progress.console.print(
                    f"  [yellow]⏭️[/] [dim]{nome_arquivo}[/] - fora do padrao"
                )
                pulados_padrao += 1
            ...
            if destino.exists():
                ...
                if not sobrescrever:
                    progress.console.print(
                        f"  [yellow]⏭️[/] [dim]{nome_arquivo}[/] - conflito, pulado"
                    )
                    pulados_conflito += 1
                    progress.advance(task_principal)
                    continue
    ...
    tabela.add_row("[green]✅[/]", f"Processados:  [green]{processados}[/]")
    tabela.add_row("[yellow]⏭️[/]", f"Pulados (fora do padrao):  [yellow]{pulados_padrao}[/]")
    tabela.add_row("[yellow]⏭️[/]", f"Pulados (conflito):  [yellow]{pulados_conflito}[/]")
    tabela.add_row("[red]❌[/]", f"Erros:  [red]{erros}[/]")
    tabela.add_row("", "")
    tabela.add_row("[blue]📂[/]", f"Diretorio:  [cyan]{dir_saida}[/]")
